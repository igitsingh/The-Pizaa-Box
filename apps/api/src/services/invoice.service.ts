...
export class InvoiceService {
    ...
    async generateInvoice(order: any): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            ...
            doc.text(`Invoice Date: ${order.invoiceGeneratedAt ? format(new Date(order.invoiceGeneratedAt), 'dd/MM/yyyy') : format(new Date(), 'dd/MM/yyyy')}`);
            ...
            order.items.forEach((item: any) => {
                ...
                if (item.variants && Object.values(item.variants).length > 0) {
                    variantStr = Object.values(item.variants).map((v: any) => v.label).join(', ');
                }
