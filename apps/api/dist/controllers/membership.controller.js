...
exports.calculateTier = exports.TIER_BENEFITS = exports.updateMembershipTier = exports.getAllTierBenefits = exports.getMyMembership = void 0;
...
exports.TIER_BENEFITS = TIER_BENEFITS;
// Calculate tier based on spending
const calculateTier = (lifetimeSpending) => {
    if (lifetimeSpending >= TIER_THRESHOLDS.PLATINUM)
        return 'PLATINUM';
    if (lifetimeSpending >= TIER_THRESHOLDS.GOLD)
        return 'GOLD';
    if (lifetimeSpending >= TIER_THRESHOLDS.SILVER)
        return 'SILVER';
    return 'BRONZE';
};
...
const getAllTierBenefits = async (req, res) => {
    try {
...
exports.getAllTierBenefits = getAllTierBenefits;
// POST /api/membership/update-tier - Update user tier (called after order)
const updateMembershipTier = async (userId, orderAmount) => {
    try {
        const user = await prisma.user.findUnique({
            where: { id: userId }
        });
...
exports.updateMembershipTier = updateMembershipTier;
